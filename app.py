...
def aplicar_ocr_pdf(file_content):
    try:
        ...
        for i, pagina in enumerate(paginas):
            # Salvar a imagem para verificar visualmente
            pagina_path = f'/tmp/page_{i}.png'
            pagina.save(pagina_path)
            logging.debug(f"Imagem da página salva em: {pagina_path}")
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            if file_extension == 'pdf':
                ...
                if not dados:
                    return jsonify({'error': 'Nenhum dado extraído do DOCX'}), 500
